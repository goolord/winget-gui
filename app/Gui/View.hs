...
import NanoUI.Testing (textFieldActive)
...
newViewCache = ViewCache <$> newIORef Nothing <*> newIORef Nothing <*> newIORef Nothing <*> newIORef Nothing
...
appView env cache = do
  ...
  -- Escape closes a dialog or clears the selection.
  ...
  when (inputKeysElem KeyEscape keys) $
    if anyModal
      ...
      else unless editing (setSelection (const Set.empty))
        ...
        Just r
          | not anyModal && rectContains r pos ->
              let i = floor ((v2Y pos - rectY r + y) / rowH)
                  localX = v2X pos - rectX r
               in if i >= 0 && i < n && localX > checkW && localX < rectW r - actionW - 24
    ...
    panelStyledWith (palSurface pal) (palSurface pal) (fillW . tight) $
      columnWith (fillW . gap 0 . tight) $ do
        ...
        rowWith (fillW . padXY 16 12 . gap 10 . alignMid . tight) $ do
          labelWith (tight . fontSemiBold . fontSize 20 . fontColor (palText pal)) "Installed apps"
          spacer (Fixed 8) Fit
          q <- rowWith (tight . fixedW 340) (searchField "Search name, id, or publisher" query)
          when (q /= query) (setQuery q)
          f <- selectWith (fixedW 190) (map filterLabel [minBound .. maxBound]) (fromEnum filt)
          when (f /= fromEnum filt) (setFilt (toEnum f))
          let diskIx = maybe 0 (\d -> maybe 0 (+ 1) (findIndex ((== d) . dsDrive) disks)) disk
          k <- selectWith (fixedW 300) ("All disks" : map diskLabel disks) diskIx
          when (k /= diskIx) $
        ...
        rowWith (fillW . fixedH 42 . padXY 16 0 . gap 12 . alignMid . tight) $ do
        ...
        rowWith (fillW . fixedH 40 . gap 10 . alignMid . padLR (14 + listInset) (14 + rightInset) . tight) $ do
  ...
  (confirmResp, confirmAct) <- modal (isJust pending) (maybe "" (\pb -> verb (pbKind pb) <> " apps") pending) $
    forM pending $ \pb -> columnWith (gap 10 . minW 540 . dialogBody) $ do
  ...
  (rulesResp, rulesAct) <- modal rulesOpen "Select by rule" $ columnWith (gap 8 . minW 640 . dialogBody) $ do
  ...
  panelStyledWith background background (fillW . fixedH rowH . tight) $
    rowWith (fillW . fixedH rowH . gap 10 . alignMid . padXY 14 0 . tight) $ do
      ...
      columnWith (grow . minW 220 . gap 3 . alignMid . cellPad . tight) $ do
        ...
        labelWith (tight . fontSize 12 . fontColor (palTextFaint pal)) (ellipsize 72 (pkgId p))
      rowWith (fixedW publisherW . alignMid . cellPad . tight) $
        labelWith (tight . fontColor (palTextMuted pal)) (ellipsize 26 (pkgPublisher p))
      columnWith (fixedW versionW . gap 3 . alignMid . cellPad . tight) $ do
        labelWith (tight . fontSize 13 . fontColor (palTextMuted pal)) (ellipsize 20 version)
        unless (T.null (pkgAvailable p)) $
          labelWith (tight . fontSize 12 . fontColor (palGreen pal)) ("↑ " <> ellipsize 18 (pkgAvailable p))
...
detailsBody p = columnWith (gap 6 . minW 600 . dialogBody) $ do
  let pal = palette
...
queuePanel env st =
  panelStyledWith (palSurface pal) (palSurface pal) (fillW . fixedH 240 . tight) $
    columnWith (fillW . fillH . padXY 16 10 . gap 8 . tight) $ do
      ...
      scrollWith (fillW . fillH) $ columnWith (fillW . gap 2 . tight) $
        V.forM_ jobs $ \jv -> withKey (jobToken (jvJob jv)) $
          rowWith (fillW . fixedH 30 . gap 10 . alignMid . tight) $ do
            let job = jvJob jv
            labelWith (tight . fixedW 76 . fontColor (palTextFaint pal)) (if jobKind job == OpUpgrade then "Upgrade" else "Uninstall")
            labelWith (tight . fixedW 300 . fontColor (palText pal)) (ellipsize 42 (pkgName (jobPackage job)))
            ...
            rowWith (fixedW 80 . alignMid . tight) $ case jvStatus jv of
